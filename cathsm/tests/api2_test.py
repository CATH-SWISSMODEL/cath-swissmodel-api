...
class SMAlignmentManagerTest(unittest.TestCase):
    ...
    def test_defaults(self):
        ...
        self.assertTrue(os.path.isfile(outfile1.name),
                        'outfile exists (user auth)')
        self.assertEqual(len(atom_lines(outfile1.name)),
                         2187, 'correct number of atoms')
        ...
        self.assertTrue(os.path.isfile(outfile2.name),
                        'outfile exists (token auth)')
        self.assertEqual(len(atom_lines(outfile2.name)),
                         2187, 'correct number of atoms')
